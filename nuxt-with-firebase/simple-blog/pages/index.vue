            ...
            <div class="w3-col l8 s12">
                <div v-for="post in allPosts" :key="post.id">
                    ...
                    <div class="w3-card-4 w3-margin w3-white">
                        <img :src="post.imgPath" alt="Nature" style="width:100%">
                        <div class="w3-container">
                            <h3><b>{{ post.title }}</b></h3>
                            <h5>{{ post.description }}, <span class="w3-opacity">{{ post.createdOn }}</span></h5>
                        </div>
                        <div class="w3-container">
                            <p>{{ post.content | trimContent }}</p>
                            <div class="w3-row">
                                <div class="w3-col m8 s12">
                                    <p><button class="w3-button w3-padding-large w3-white w3-border"><b>READ MORE »</b></button></p>
                                </div>
                                <div class="w3-col m4 w3-hide-small">
                                    <p><span class="w3-padding-large w3-right"><b>Likes  </b> <span class="w3-tag">{{ post.likes }}</span></span></p>
                                </div>
                            </div>
                        </div>
                    </div>
...
const { postsCollection } = require("@/firebase.config.js")
...
export default {
    ...
    async asyncData() {
        ...
        await allPosts.forEach(
            post => {
                // Format date
                if (!post.createdOn) {
                    post.createdOn = "-"
                } else {
                    let date = post.createdOn.toDate()
                    post.createdOn = moment(date).fromNow()
                }
